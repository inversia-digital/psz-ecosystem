...
export function brandMarkSvg(opts: {
  ...
  // Con firma:
  //  - T/P baja (línea base 56→67) para dejar el MISMO hueco con la
  //    horizontal (y=74) que el que "E·242" tiene por debajo.
  //  - La barra vertical se recorta por ARRIBA lo mismo que baja T/P
  //    (y1 20→31) y baja hasta casi la horizontal (y2=70) → ⊥.
  // Sin firma (favicon): geometría original (56 / 20 / 62).
  const vBottom = showText ? 70 : 62
  const vTop = showText ? 31 : 20
  const tpY = showText ? 67 : 56

  return (
    `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 100 100">` +
    `<line x1="50" y1="${vTop}" x2="50" y2="${vBottom}" stroke="${accent}" stroke-width="0.8"/>` +
    `<text x="44" y="${tpY}" text-anchor="end" font-size="40" font-weight="500" font-family="${SERIF}" fill="${tColor}">T</text>` +
    `<text x="56" y="${tpY}" text-anchor="start" font-size="40" font-weight="500" font-family="${SERIF}" fill="${pColor}">P</text>` +
    reg +
    `</svg>`
  )
}
...
export function BrandMark({
  ...
  const { tColor, pColor, accent, e242 } = resolveColors(bg, mono, inverse)
  // Con firma: T/P baja (56→67) para igualar su hueco con la horizontal
  // al que "E·242" tiene debajo; la barra vertical se recorta por arriba
  // lo mismo (20→31) y baja a y2=70 → ⊥. Sin firma: original (56/20/62).
  const vBottom = showText ? 70 : 62
  const vTop = showText ? 31 : 20
  const tpY = showText ? 67 : 56
  return (
    ...
    >
      <line x1="50" y1={vTop} x2="50" y2={vBottom} stroke={accent} strokeWidth="0.8" />
      <text
        x="44"
        y={tpY}
        textAnchor="end"
